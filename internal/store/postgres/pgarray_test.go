...
func TestTextArrayScan(t *testing.T) {
	tests := []struct {
		...
		{"unquoted whitespace trimmed", "{ admin , read_zones }", TextArray{"admin", "read_zones"}, false},
		{"quoted whitespace preserved", `{" admin ",b}`, TextArray{" admin ", "b"}, false},
		{"unquoted NULL rejected", "{admin,NULL}", nil, true},
		{"quoted NULL kept", `{"NULL",admin}`, TextArray{"NULL", "admin"}, false},
